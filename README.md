...
| --- | --- |
| `/` | User stories hub |
| `/user-stories/<slug>/` | Published user story writeup |

## Drafts

Keep unfinished stories under `drafts/` in git. That folder is listed in
`.vercelignore`, so drafts are **not deployed** to userstories.clinicaltoken.com.
Do not link drafts from the public hub. Promote a draft by moving it to
`/user-stories/<slug>/` and adding a hub card.

Current draft in repo:

| Path | Purpose |
| --- | --- |
| `drafts/evidence-exchange-monetize-deidentified-findings/` | Sequel on Evidence Exchange publish + monetization (video demo 5) |
...
- Split discovery stories from monetization / Evidence Exchange stories when both themes compete for attention.
- Do not publish draft stories on the public hub until they are ready.
